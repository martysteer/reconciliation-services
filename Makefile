...
SERVICES ?= fast geonames isolang iso15924 rbmscv problemlcsh clc
...
.PHONY: data data-fast data-geonames data-isolang data-iso15924 data-rbmscv \
        data-problemlcsh data-clc build up down logs status save clean clean-data
...
data-clc:
	$(MAKE) -C services/clc build
	@mkdir -p data
	cp services/clc/data/clc.db data/clc.db
